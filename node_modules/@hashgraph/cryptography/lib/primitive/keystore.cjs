...
async function createKeystore(privateKey, passphrase) {
  ...
  const salt = _tweetnacl.default.randomBytes(saltLen);

  const key = await pbkdf2.deriveKey(hmac.HashAlgorithm.Sha256, passphrase, salt, c, dkLen);

  const iv = _tweetnacl.default.randomBytes(16); // AES-128-CTR with the first half of the derived key and a random IV
