...
class Mnemonic {
  ...
  static async _generate(length) {
    ...
    const entropy = _tweetnacl.default.randomBytes(neededEntropy);
  ...
  async _toLegacyPrivateKey() {
    ...
    const keyData = await pbkdf2.deriveKey(hmac.HashAlgorithm.Sha512, password, salt, 2048, 32);
    return _PrivateKey.default.fromBytes(keyData);
  }
  /**
   * @private
  ...
  _toLegacyEntropy() {
    const indicies = this.words.map(word => _legacy.default.indexOf(word.toLowerCase()));

    const data = _convertRadix(indicies, _legacy.default.length, 256, 33);
