...
class Hbar {
  ...
  constructor(amount, unit = _HbarUnit.default.Hbar) {
    if (unit === _HbarUnit.default.Tinybar) {
      ...
      if (amount instanceof _long.default) {
        bigAmount = new _bignumber.default(amount.toString(10));
      } else if (_long.default.isLong(amount)) {
        bigAmount = new _bignumber.default(_long.default.fromValue(amount).toString(10));
      ...
      this._valueInTinybar = _long.default.fromString(bigAmount.multipliedBy(unit._tinybar).toFixed());
    }
  }
  /**
